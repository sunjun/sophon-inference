...
class DetectorDelegate {
 ...
 public:
  ...
  void set_next_inference_pipe(BMInferencePipe<T> *nextPipe) {
    m_nextInferPipe = nextPipe;
  }
  int set_detected_callback(DetectedFinishFunc func) {
...
class BMInferencePipe {
 ...
 public:
  ...
  int init(const DetectorParam &param,
           std::shared_ptr<DetectorDelegate<T>> delegate) {
    m_param = param;
    m_detect_delegate = delegate;

    m_preprocessWorkerPool.init(&m_preprocessQue, param.preprocess_thread_num,
    ...
    m_postprocessWorkerPool.init(&m_postprocessQue,
                                 param.postprocess_thread_num, 1, 8);
    m_postprocessWorkerPool.startWork([this](std::vector<T> &items) {
      if (m_postprocessQue.size() > m_param.postprocess_queue_size) {
        std::cout << "WARNING:postprocess queue_size("
                  << m_postprocessQue.size() << ") > "
  ...
  int flush_frame() { return m_preprocessWorkerPool.flush(); }

  int push_frame(T *frame) { return m_preprocessQue.push(*frame); }
};
}  // end namespace bm
